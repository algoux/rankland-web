...
describe('collection loading state parity', () => {
  it('uses the legacy Ant Design spinner with pt-16 text-center state styling', () => {
    ...
    expect(source).toContain('<a-spin v-else-if="!collection" data-id="collection-loading" class="collection-state" />');
    expect(source).not.toContain('data-id="collection-loading" class="collection-state">\n      Loading');
