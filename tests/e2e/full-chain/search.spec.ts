...
test.describe('/search full-chain route', () => {
  test('shows recent ranklists for an empty query through CSR and listAllRanklists', async ({ page, request }) => {
    ...
    await expect(page.locator('[data-id="search-hydrated"]')).toHaveAttribute('aria-hidden', 'true');
    await expect(page.locator('[data-id="search-hydrated"]')).toHaveCSS('position', 'absolute');
