# User Modal Unofficial Line Product Class Parity Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Remove the Vue-only product class from the user modal non-official notice line while preserving old React exact `mt-4 mb-0` classes and spacing.

**Architecture:** Keep the shared Vue SRK user modal and stable `data-id` selector. Add full-chain RED assertions for the exact unofficial line class list, then retarget scoped unofficial-line styles to `data-id` plus the old utility classes.
...
- Create: `docs/superpowers/specs/2026-05-28-user-modal-unofficial-line-product-class-parity-design.md`
- Create: `docs/superpowers/plans/2026-05-28-user-modal-unofficial-line-product-class-parity.md`

## Tasks

- [x] Create the design spec and implementation plan.
- [x] Add RED full-chain assertions for the exact unofficial line class list.
- [x] Run focused Ranklist full-chain test and confirm expected failure.
- [x] Remove Vue-only unofficial line product class.
...
Expected RED: focused Ranklist full-chain test fails because current Vue still emits `rankland-user-modal-unofficial mt-4 mb-0`.

Observed RED: the focused Ranklist full-chain test failed because `[data-id="rankland-user-modal-unofficial"]` still reported `rankland-user-modal-unofficial mt-4 mb-0` instead of exact old React `mt-4 mb-0`.
...
```less
[data-id='rankland-user-modal-unofficial'].mt-4.mb-0 {
  margin: 16px 0 0;
}
```
...
Commit only the Vue SRK wrapper, Ranklist full-chain test, this spec/plan, and migration documentation for the user modal unofficial-line product-class parity slice.
