# Playground Canonical Link Omission Parity Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Restore old React `/playground` Head output by removing the Vue-only route-level canonical link.

**Architecture:** Keep Playground Head ownership in `playground.view.vue`. The route still declares title and `og:title`, while app-level document defaults remain outside this slice.
...
- [x] **Step 1: Assert no Playground canonical link**

Add this assertion after the title check in `hydrates the CSR playground and previews bundled SRK without upstream calls`:
...
Record this slice as `Playground canonical link omission parity`, including:
...
```bash
git add src/client/modules/playground/playground.view.vue tests/e2e/full-chain/playground.spec.ts docs/migration/status.md docs/migration/manual-acceptance-checklist.md docs/migration/final-integration-review.md docs/superpowers/specs/2026-05-28-playground-canonical-link-omission-parity-design.md docs/superpowers/plans/2026-05-28-playground-canonical-link-omission-parity.md
git commit -m "fix: 移除 Playground canonical 链接"
```

Observed: committed as `fix: 移除 Playground canonical 链接`.
...
Observed: clean branch status on `migration/live-page-foundation`, `git show --check --oneline HEAD` reported `fix: 移除 Playground canonical 链接`, `git diff --check` produced no output, and the process scan showed no lingering Playwright/Vite/mock server process.
