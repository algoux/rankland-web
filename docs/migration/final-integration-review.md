...
| Final migration gate | `corepack pnpm test:migration` passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 52 full-chain Playwright tests during the 2026-05-26 home recommendation card heading/icon parity pre-commit verification. | Verified |
...
- Home: broader SEO/content polish beyond verified structured data, SSR content, Ant Design content layout, recommendation card heading/icon parity, and contact modal behavior remains accepted as non-blocking.
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 52 full-chain Playwright tests during the home recommendation card heading/icon parity verification
- `git diff --check`: passed in the latest verified home recommendation card heading/icon parity slice
