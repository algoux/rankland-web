...
| Playground page parity | `tests/e2e/full-chain/playground.spec.ts` covers CSR hydration, absence of the Vue-only page-level `Playground` toolbar/heading, hidden hydration marker, hidden editor-ready marker, Monaco readiness/schema/theme, one-time welcome modal, preview docs link/icon placement, invalid JSON state, render-error Alert, bundled preview rendering, desktop/mobile viewport bounds, and no upstream calls. | Verified |
...
| Final migration gate | 2026-05-27 playground toolbar chrome parity slice passed focused Playground RED/GREEN and `corepack pnpm test:migration` with build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests; `git diff --check` also passed. | Verified |
...
- Playground: exact old Monaco `0.34.0` package-version parity is intentionally not preserved because the verified Vue wrapper requires Monaco `>=0.43.0`; synthetic Monaco editing remains a harness limitation, while the product editor path, absence of the Vue-only toolbar/heading, hidden hydration/editor-ready markers, preview-pane `QuestionCircleOutlined` docs link, and stable preview coverage are verified by build/hydration and full-chain tests.
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests during the playground toolbar chrome parity verification
