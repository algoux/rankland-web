...
| Final migration gate | 2026-05-27 user modal root class parity slice passed focused Ranklist RED/GREEN and `corepack pnpm test:migration` with build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests; `git diff --check` also passed. | Verified |
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests during the user modal root class parity verification
