...
| Final migration gate | 2026-05-27 search load error color parity slice passed focused RED/GREEN and `corepack pnpm test:migration` with build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests; `git diff --check` also passed. | Verified |
...
- Search: old load-error `text-red-500` color is now verified alongside prior normal-content, Ant Design Input.Search/List/Spin, hydration marker, recent/result rows, and zero-result behavior.
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests during the search load error color parity verification
