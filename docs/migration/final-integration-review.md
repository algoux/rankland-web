...
| Final migration gate | 2026-05-27 srk header title typography parity slice passed focused RED/GREEN and `corepack pnpm test:migration` with build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 53 full-chain Playwright tests; `git diff --check` also passed. | Verified |
...
- SRK renderer wrapper: old header title typography is now verified at 32px/500/4px; remaining lower-level exact table pixel parity should be handled only when product review identifies a concrete difference.
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 53 full-chain Playwright tests during the srk header title typography parity verification
