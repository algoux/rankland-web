...
| Final migration gate | 2026-05-27 collection nav legacy chrome parity slice passed focused RED/GREEN and `corepack pnpm test:migration` with build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests; `git diff --check` also passed. | Verified |
...
- Collection: old light `#f4f4f4` and dark `#111111` nav background, dark `#434343` nav border, and old `.srk-collection-hidden-header h3.mb-0` hidden-title DOM are now verified alongside prior Ant Design Vue menu/category icon/mobile collapse/remaining-height behavior.
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests during the collection nav legacy chrome parity verification
