...
| Final migration gate | 2026-05-26 home card dark Ant Design parity slice passed focused RED/GREEN, home full-chain 2/2, and `corepack pnpm test:migration` with build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 52 full-chain Playwright tests; `git diff --check` also passed. | Verified |
...
- Home: broader SEO/content polish beyond verified structured data, SSR content, Ant Design content layout, recommendation card dark Ant Design Card token parity, recommendation card heading/icon parity, card title icon/logo spacing parity, paste.then.ac logo padding parity, and shared Ant Design Vue contact modal behavior remains accepted as non-blocking.
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 52 full-chain Playwright tests during the home card dark Ant Design parity verification
