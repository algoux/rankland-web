...
| Final migration gate | 2026-05-27 playground flex layout parity slice passed focused Playground RED/GREEN, viewport GREEN, and `corepack pnpm test:migration` with build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests; `git diff --check` also passed. | Verified |
...
- Playground: exact old Monaco `0.34.0` package-version parity is intentionally not preserved because the verified Vue wrapper requires Monaco `>=0.43.0`; synthetic Monaco editing remains a harness limitation, while the product editor path, old desktop flex/500px layout, absence of the Vue-only toolbar/heading and visible Preview button, hidden hydration/editor-ready markers, preview-pane `QuestionCircleOutlined` docs link, and stable preview coverage are verified by build/hydration and full-chain tests. Mobile keeps the migrated no-horizontal-overflow guard.
- SRK renderer wrapper: old header title typography is now verified at 32px/500/4px, old header meta/contributors/ref-links/time text size is verified at 14px, old header export/share action icons are verified with light/dark primary plus hover colors, and user-modal rank-time is restored to the old chart-only `mt-4` wrapper while preserving G2 tooltip/animation behavior; remaining lower-level exact table pixel parity should be handled only when product review identifies a concrete difference.
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests during the playground flex layout parity verification
