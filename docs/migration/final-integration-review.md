...
| App shell/layout parity | `tests/e2e/full-chain/app-shell.spec.ts` covers Ant Design Vue Layout/Menu/Dropdown/Button shell structure, navigation, site switch with old ArrowRight icon, desktop legacy 50px header padding/no centered max-width/no inner gap, mobile legacy 20px header padding/no inner padding/no inner gap, 64px header/logo container, 40px logo image, 16px nav item padding, 8px site-switch padding, focus-mode shell bypass, pre-hydration theme bootstrap, theme/macOS class sync, legacy `Title \| RankLand` document-title behavior on CSR navigation, analytics dispatch, fallback 404, and desktop/mobile viewport bounds. | Verified |
...
| Final migration gate | `corepack pnpm test:migration` passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 52 full-chain Playwright tests during the 2026-05-26 SRK header meta legacy spacing parity pre-commit verification. | Verified |
...
- Home: broader SEO/content polish beyond verified structured data, SSR content, Ant Design content layout, recommendation card heading/icon parity, card title icon/logo spacing parity, paste.then.ac logo padding parity, and contact modal behavior remains accepted as non-blocking.
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 52 full-chain Playwright tests during the SRK header meta legacy spacing parity verification
- `git diff --check`: passed in the latest verified SRK header meta legacy spacing parity slice
