...
| Final migration gate | 2026-05-27 user modal slogan class parity slice passed focused Ranklist RED/GREEN and `corepack pnpm test:migration` with build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests; `git diff --check` also passed. | Verified |
...
- SRK renderer wrapper: old header title typography is now verified at 32px/500/4px, old header meta/contributors/ref-links/time text size is verified at 14px, old header export/share action icons are verified with light/dark primary plus hover colors, and user-modal body restores the old `.user-modal` root class while the slogan restores the old `.slogan` class; rank-time is restored to the old chart-only `mt-4` wrapper and G2 tooltip/animation behavior; remaining lower-level exact table pixel parity should be handled only when product review identifies a concrete difference.
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests during the user modal slogan class parity verification
