...
| Final migration gate | 2026-05-27 live scroll toggle spacing parity slice passed focused RED/GREEN and `corepack pnpm test:migration` with build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests; `git diff --check` also passed. | Verified |
...
- Live: old scroll-solution toggle text-to-switch spacing is now verified at 4px alongside prior Ant Design Vue Switch shape/state, WebSocket reconnect/close, Toastify container, and realtime layout behavior.
...
- SRK renderer wrapper: old header title typography is now verified at 32px/500/4px, old header meta/contributors/ref-links/time text size is verified at 14px, and old header export/share action icons are verified with light/dark primary plus hover colors; remaining lower-level exact table pixel parity should be handled only when product review identifies a concrete difference.
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests during the live scroll toggle spacing parity verification
