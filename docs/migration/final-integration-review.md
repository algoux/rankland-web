...
| App shell/layout parity | `tests/e2e/full-chain/app-shell.spec.ts` covers Ant Design Vue Layout/Menu/Dropdown/Button shell structure, navigation, site switch with old ArrowRight icon, mobile legacy 64px header/logo container, 40px logo image, 20px header padding, 16px nav item padding, 8px site-switch padding, focus-mode shell bypass, pre-hydration theme bootstrap, theme/macOS class sync, legacy `Title \| RankLand` document-title behavior on CSR navigation, analytics dispatch, fallback 404, and desktop/mobile viewport bounds. | Verified |
...
| Final migration gate | `corepack pnpm test:migration` passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 52 full-chain Playwright tests during the 2026-05-26 app shell mobile legacy metrics parity pre-commit verification. | Verified |
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 52 full-chain Playwright tests during the app shell mobile legacy metrics parity verification
- `git diff --check`: passed in the latest verified app shell mobile legacy metrics parity slice
