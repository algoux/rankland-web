...
| Final migration gate | 2026-05-27 playground editor shell chrome parity slice passed focused Playground RED/GREEN and `corepack pnpm test:migration` with build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests; `git diff --check` also passed. | Verified |
...
- Playground: exact old Monaco `0.34.0` package-version parity is intentionally not preserved because the verified Vue wrapper requires Monaco `>=0.43.0`; synthetic Monaco editing remains a harness limitation, while the product editor path, old desktop flex/500px layout, no Vue-only editor wrapper border/radius, invalid prompt shortcut tag `mr-0` spacing, absence of the Vue-only toolbar/heading and visible Preview button, hidden hydration/editor-ready markers, preview-pane `QuestionCircleOutlined` docs link, and stable preview coverage are verified by build/hydration and full-chain tests. Mobile keeps the migrated no-horizontal-overflow guard.
...
- `corepack pnpm test:migration`: passed build, 35 unit files / 151 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 54 full-chain Playwright tests during the playground editor shell chrome parity verification
