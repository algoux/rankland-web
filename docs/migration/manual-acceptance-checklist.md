...
```text
2026-05-27 最新记录：SRK header action gap parity 已通过 focused RED/GREEN、完整 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、57 full-chain Playwright tests）和 `git diff --check`；focused RED 复现 header meta/actions `column-gap`/`row-gap` 为 `8px`，GREEN 验证移除 Vue-only flex gap 后恢复旧版无额外 gap。
```
