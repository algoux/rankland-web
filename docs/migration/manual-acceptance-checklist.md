...
```text
2026-05-28 最新记录：SRK share link hash omission parity 已通过 focused RED/GREEN、focused ranklist full-chain、完整默认 `test:migration` 与 `git diff --check`。RED 复现 `normalizeRanklandShareUrl` 仍保留 `#scoreboard`；GREEN 验证复制 SRK 页面链接时过滤旧版 focus-only query keys，并像旧 React `useCurrentUrl` 一样不复制 URL fragment，同时保留其他 query 参数。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client route，`test:migration` 通过 build、36 个 unit 文件 / 154 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests。
```
...
- `[x]` rank-time 面板 class 精确为旧版 `mt-4`，不渲染 Vue-only `rankland-rank-time-panel`，不展示额外单位、摘要或过题事件 chip，G2 曲线可接受
- `[x]` 导出 SRK 可用
- `[x]` 导出 Gym Ghost 可用
