...
```text
2026-05-27 最新记录：SRK contributor link rel parity 已通过 focused RED/GREEN、完整 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 full-chain Playwright tests）和 `git diff --check`；focused RED 复现 URL contributor 链接仍渲染 `rel="noreferrer"` 而旧 React 为 `rel="noopener"`，GREEN 验证 `Name (url)` 贡献者链接恢复旧版 `target="_blank"` / `rel="noopener"` DOM，同时保留纯文本 contributor、item-level `span`、逗号分隔、文本、颜色、字号和间距。
```
