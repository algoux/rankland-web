...
```text
2026-05-27 最新记录：SRK header ref-link rel parity 已通过 focused RED/GREEN、完整 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 full-chain Playwright tests）和 `git diff --check`；focused RED 复现可见 SRK header reference link 仍渲染 `rel="noreferrer"` 而旧 React 为 `rel="noopener"`，GREEN 验证可见主链接和隐藏 dropdown 链接均恢复旧版 `target="_blank"` / `rel="noopener"` DOM，同时保留链接文本、颜色、间距、item wrapper 和 dropdown 行为。
```
