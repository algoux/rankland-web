...
```text
2026-05-28 最新记录：Collection state wrapper DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration` 与 `git diff --check`。RED 复现 `/collection/:id` collection-level NotFound/error 使用 `SECTION.collection-state.pt-16.text-center`，selected-ranklist error 额外包含 `collection-state`，collection-level/selected-ranklist loading 直接以 Ant Design Spin 作为状态根节点，缺少旧 React plain `DIV.pt-16.text-center` 外壳；GREEN 验证 collection-level NotFound/error/loading 与 selected-ranklist error/loading 均恢复为精确 `DIV.pt-16.text-center`，不再渲染 Vue-only `collection-state` class，loading 内部包含 Ant Design Spin 子节点，并且 collection desktop/mobile bounds 仍通过。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client route，`test:migration` 通过 build、38 个 unit 文件 / 158 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、61 passed / 1 skipped full-chain Playwright tests。
```
...
最新自动化结论：Collection state wrapper DOM parity 已纳入收口记录；focused RED/GREEN 已通过，完整 gate 已通过，`git diff --check` 已通过。
