...
```text
2026-05-27 最新记录：Search result count DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、59 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 Vue-only `/search` 结果数内层 span，result summary 仍渲染 `[data-id="search-result-count"]`；GREEN 验证结果数恢复旧 React 纯文本 `搜索到 N 个结果`，不渲染 `[data-id="search-result-count"]`，并保留 result section 的 `data-result-count`。
```
