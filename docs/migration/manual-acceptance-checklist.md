...
```text
2026-05-28 最新记录：user modal rank-time panel product class parity 已通过 focused RED/GREEN、live full-chain 11 tests、完整默认 `test:migration` 与 `git diff --check`。RED 复现迁移版 rank-time panel 仍暴露 Vue-only `rankland-rank-time-panel` class；GREEN 验证 rank-time panel 恢复旧 React exact `mt-4` class list，且 stable `data-id`、chart-only chrome、无 unit/summary/event 节点、16px 顶部间距、0px padding/border、G2 metadata、canvas 和曲线高度保持不变；完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client routes，`test:migration` 通过 build、36 个 unit files / 154 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests。
```
