...
```text
2026-05-27 最新记录：SRK extra ref-link spacing parity 已通过 focused RED/GREEN、完整 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、57 full-chain Playwright tests）和 `git diff --check`；focused RED 复现 hidden ref-link trigger `margin-left: 4px`，GREEN 验证恢复为 `0px`。
```
