...
```text
2026-05-28 最新记录：App document body bootstrap parity 已通过 focused RED/GREEN、app-shell full-chain 8 tests、完整默认 `test:migration` 与 `git diff --check`。RED 复现迁移版 raw app document 缺少旧版 inline body bootstrap style 和 Dark Reader 清理脚本；GREEN 验证旧 React document body 启动行为已恢复：inline body bootstrap style 位于 `<body>` 前、Dark Reader 清理脚本位于 app mount 前，app CSS 加载后 `body` opacity 恢复为 `1`。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client route，`test:migration` 通过 build、36 个 unit 文件 / 154 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests。
```
