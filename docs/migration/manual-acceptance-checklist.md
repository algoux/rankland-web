...
```text
2026-05-27 最新记录：Playground preview wrapper DOM/class parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；focused RED 复现 Playground 有效 SRK 预览外壳仍是 `section.playground-preview`，GREEN 验证预览外壳恢复旧版 `div.mt-8.mb-8`、32px 上下间距、SRK 预览、筛选、进度、文档链接和无上游请求行为。
```
