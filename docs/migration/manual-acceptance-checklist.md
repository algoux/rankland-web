...
```text
2026-05-28 最新记录：Ranklist view-count class parity 已通过 focused RED/GREEN、ranklist full-chain 9 tests、完整默认 `test:migration`（build、35 unit files / 152 unit tests、1 SSR smoke test、1 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`。RED 复现 view-count classList 仍包含 Vue-only `.rankland-ranklist-view-count`；GREEN 验证浏览量节点仅保留旧版 `mr-2` class 合同、不渲染该 Vue-only 产品类，并保留 Eye icon、`42` / `-` fallback、14px 字号、light/dark metadata color 和 header action separator 行为。
```
...
最新自动化结论：Ranklist view-count class parity 已纳入收口记录，完整 gate 已通过。
