...
```text
2026-05-28 最新记录：Playground Monaco minimap parity 已通过 focused RED/GREEN、Playground full-chain 7 tests、完整默认 `test:migration`（生成 6 条 client route、build、35 unit files / 153 unit tests、1 SSR smoke test、1 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`。RED 复现迁移版 Playground 隐藏 Monaco minimap；GREEN 验证 Playground 只保留旧 React 的 Monaco option `selectOnLineNumbers: true`，`.minimap` 可见且宽度大于 0，同时保留既有编辑器/预览 parity 覆盖。
```
...
最新自动化结论：Playground Monaco minimap parity 已纳入收口记录，完整 gate 已通过。
