...
```text
2026-05-28 最新记录：SRK header actions wrapper product class parity 已通过 focused RED/GREEN、ranklist + live full-chain 20 tests、完整默认 `test:migration` 与 `git diff --check`。RED 复现迁移版 header actions wrapper 仍暴露 Vue-only `rankland-ranklist-header-actions` class；GREEN 验证 header actions wrapper 保留稳定 `data-id`，class list 为空，且 `display: inline`、normal gaps、旧 export/share trigger utility classes、Ant Design Vue dropdown 行为和 metadata/no-metadata separator chrome 保持不变。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client route，`test:migration` 通过 build、36 个 unit 文件 / 154 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests。
```
...
最新自动化结论：SRK header actions wrapper product class parity 已纳入收口记录，完整 gate 已通过，`git diff --check` 已通过。
