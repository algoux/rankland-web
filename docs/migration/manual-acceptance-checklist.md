...
```text
2026-05-27 最新记录：Search error message DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、59 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 Vue-only `/search` 错误消息类，内层消息带 `.search-error-message text-red-500`；GREEN 验证搜索页错误消息恢复旧 React 精确 `div.text-red-500` 类名，且无 Vue-only `.search-error-message`。
```
