...
```text
2026-05-28 最新记录：App site-switch cnn word-break parity 已通过 focused RED/GREEN、完整默认 `test:migration` 与 `git diff --check`。RED 复现 App site-switch link 无条件渲染中国站点专属 `word-break: keep-all`；GREEN 验证 `cnn` 全球站点分支不渲染该 inline style，默认中国站点 app-shell full-chain 仍保留 `word-break: keep-all`。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client route，`test:migration` 通过 build、38 个 unit 文件 / 158 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、61 passed / 1 skipped full-chain Playwright tests。
```
...
最新自动化结论：App site-switch cnn word-break parity 已纳入收口记录；focused RED/GREEN 已通过，完整 gate 已通过，`git diff --check` 已通过。
