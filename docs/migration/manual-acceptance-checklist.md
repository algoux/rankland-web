...
```text
2026-05-27 最新记录：Home tool logo class token parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 paste.then.ac 和 Algo Bootstrap 工具卡 logo 缺少旧版 `mr-3 inline-block` class token 且计算为 `display: block`，GREEN 验证旧版 `img.mr-3.inline-block` class token、`display: inline-block`、12px 右间距、paste logo 24px 尺寸和 2px 内边距，同时保留首页行/列间距、SSR 内容、卡片行为、暗色正文色、统计请求、联系弹窗和外链语义覆盖。
```
