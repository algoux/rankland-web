...
```text
2026-05-27 最新记录：Search input DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、59 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 Vue-only `/search` 输入框类，Ant Design search wrapper 带 `.search-input`；GREEN 验证搜索框恢复旧 React 无页面自定义 class DOM，不渲染 `.search-input`，并保留 0px wrapper 顶部间距。
```
