...
```text
2026-05-28 最新记录：user modal photo inline width parity 已通过 focused RED/GREEN、ranklist full-chain 9 tests、完整默认 `test:migration` 与 `git diff --check`。RED 复现迁移版 user photo image 缺少旧 React inline `width: 100%` 样式；GREEN 验证 `[data-id="rankland-user-modal-photo"]` 恢复 inline `width: 100%`，computed full-width photo 行为、`max-width: 100%` 和 broken-image hiding 覆盖保持不变。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，生成 6 个 client routes，build、36 个 unit files / 154 个 unit tests、1 个 SSR smoke test、1 个浅层 Playwright test、60 passed / 1 skipped full-chain Playwright tests 均通过。
```
...
最新自动化结论：user modal photo inline width parity 已纳入收口记录，完整 gate 已通过。
