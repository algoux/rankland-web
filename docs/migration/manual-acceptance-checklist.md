...
```text
2026-05-28 最新记录：Collection container chrome parity 已通过 focused RED/GREEN、collection full-chain 11 tests、完整默认 `test:migration` 与 `git diff --check`。RED 复现迁移版 loaded collection root 仍带 Vue-only `position` / `min-height` chrome；GREEN 验证 `[data-id="collection-content"]` 保留旧版 `DIV.srk-collection-container`，同时 computed `position` 为 `static`、`min-height` 为 `0px`；nav、hidden header、ranklist panel、remaining-height/mobile collapse 合同保持通过。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client route，`test:migration` 通过 build、36 个 unit 文件 / 154 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests。
```
