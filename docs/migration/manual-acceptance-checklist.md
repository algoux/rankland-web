...
```text
2026-05-28 最新记录：Ranklist header action anchor parity 已通过 focused RED/GREEN、ranklist full-chain 9 tests、live full-chain 11 tests、完整默认 `test:migration`（build、35 unit files / 152 unit tests、1 SSR smoke test、1 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`。RED 复现 header 导出/分享触发器仍是 Ant Design Vue `BUTTON`，带 `ant-btn` 和 Vue-only action classes；GREEN 验证触发器还原为旧版无 `href` `A` 锚点，保留旧 utility class + Ant Design runtime `ant-dropdown-trigger`，不渲染 `ant-btn` / `ant-btn-sm` / `.rankland-ranklist-header-action-trigger` / `.rankland-ranklist-header-action-separated`。
```
