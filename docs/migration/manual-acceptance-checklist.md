...
```text
2026-05-27 最新记录：Playground docs link anchor class parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；focused RED 复现 Playground 预览区 srk 文档链接锚点仍带 Vue-only `playground-docs-link` 类，GREEN 验证 docs link 锚点为旧版 plain anchor、外壳精确类名 `absolute right-4 top-4`、`target="_blank"` 且无 `rel`、href、文本、问号图标、右上角位置、预览和无上游请求行为。
```
