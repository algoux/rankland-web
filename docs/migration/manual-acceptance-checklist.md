...
```text
2026-05-27 最新记录：SRK contributor item span DOM parity 已通过 focused RED/GREEN、完整 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 full-chain Playwright tests）和 `git diff --check`；focused RED 复现第二个可见 SRK header contributor 被拆成逗号 `SPAN` 与文本 `SPAN` 两个 direct children，GREEN 验证 `https://github.com/rankland-alpha` 和 `, Team Beta` 均恢复为旧 React item-level `span` 项，同时保留外层 `p.mb-0`、文本、链接处理、颜色、间距和相关链接行为。
```
