...
```text
2026-05-27 最新记录：user modal empty organization line parity 已通过 focused RED/GREEN、完整 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 full-chain Playwright tests）和 `git diff --check`；focused RED 复现无组织用户弹窗缺少 `[data-id="rankland-user-modal-organization"]`，GREEN 验证空组织用户仍保留旧版空 `p.mb-0` 组织行和 0px/0px 间距。
```
