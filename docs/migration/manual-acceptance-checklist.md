...
```text
2026-05-28 最新记录：App header nav wrapper parity 已通过 focused RED/GREEN、完整默认 `test:migration` 与 `git diff --check`。RED 复现 App header 将 `[data-id="app-nav"]` 直接渲染为 `.app-header-inner` 的子节点，缺少旧 React `div style={{ flex: 1, minWidth: 0 }}` 导航包裹层；GREEN 验证第二个直接子节点恢复为 `DIV`，计算样式为 `flex: 1 1 0%` / `min-width: 0px`，并且 desktop/mobile app-shell bounds 仍通过。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client route，`test:migration` 通过 build、38 个 unit 文件 / 158 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、61 passed / 1 skipped full-chain Playwright tests。
```
...
最新自动化结论：App header nav wrapper parity 已纳入收口记录；focused RED/GREEN 已通过，完整 gate 已通过，`git diff --check` 已通过。
