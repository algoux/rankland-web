...
```text
2026-05-27 最新记录：SRK ref-links span DOM parity 已通过 focused RED/GREEN、完整 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 full-chain Playwright tests）和 `git diff --check`；focused RED 复现 SRK header reference-link wrapper 渲染为 `P` 而不是旧 React `SPAN`，GREEN 验证 wrapper 恢复为 `span` 并保持文本、颜色、间距和 dropdown 行为。
```
...
- `[x]` 榜单相关链接 wrapper 使用旧版 `span` DOM，同时保留现有链接文本、颜色、间距和 dropdown 行为
