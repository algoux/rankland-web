...
```text
2026-05-27 最新记录：Search list item DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、59 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 Vue-only `/search` 列表项类，result/recent Ant Design list item 带 `.search-list-item`；GREEN 验证列表项恢复旧 React 无页面自定义 class DOM，不渲染 `.search-list-item`，并恢复 Ant Design list item `display: flex`。
```
