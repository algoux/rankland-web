...
```text
2026-05-28 最新记录：SRK banner image class/style parity 已通过 focused RED/GREEN、ranklist full-chain 9 tests、完整默认 `test:migration`（生成 6 条 client route、build、36 unit files / 154 unit tests、1 SSR smoke test、1 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`。RED 复现迁移版 SRK banner image 仍带 Vue-only `rankland-ranklist-banner` 产品 class；GREEN 验证 image class 精确恢复为旧版 `mb-2`，inline style 恢复 `max-width: min(100%, 1820px)` / `max-height: 40vh`，且不渲染 Vue-only 产品 class 或 scoped size CSS。
```
