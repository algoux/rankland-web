...
```text
2026-05-27 最新记录：Search heading DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、59 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 Vue-only `/search` 标题类，heading 带 `.search-heading mb-6`；GREEN 验证搜索页标题恢复旧 React 精确 `h3.mb-6` 类名，且无 Vue-only `.search-heading`，并保留 0px/24px 标题间距。
```
