...
```text
2026-05-27 最新记录：SRK table wrapper class attribute parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 ranklist/live 表格外壳包含额外 `rankland-ranklist-table-wrapper` class、collection/playground 未传 `tableClass` 时仍输出 Vue-only class，GREEN 验证 ranklist/live 精确输出旧版 `class="ml-4"`，collection/playground 保留 `data-id` 但不输出 class 属性。
```
