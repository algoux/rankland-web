...
```text
2026-05-27 最新记录：SRK modal table-wrapper DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、59 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 Vue-only modal wrapper 位置，用户/题目弹窗 wrapper 都不在 `[data-id="rankland-ranklist-table-wrapper"]` 内；GREEN 验证两个弹窗 wrapper 已恢复到旧 React table wrapper 内，同时 footer 保持在 table wrapper 外，并保留既有弹窗内容、root class、rank-time、题目弹窗和 footer 行为。
```
...
最新自动化结论：SRK modal table-wrapper DOM parity 已纳入收口记录，完整 gate 已通过。
