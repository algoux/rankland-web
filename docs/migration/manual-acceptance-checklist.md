...
```text
2026-05-27 最新记录：SRK header meta DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、59 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 Vue-only contributors/ref-links sibling DOM，两者 parent data-id 为空而非旧 React 的 `rankland-ranklist-header-meta`；GREEN 验证贡献者和相关链接已回到旧版 `div.text-center.mt-1` header meta block 内，同时保留既有文本、span DOM、链接属性、颜色、dropdown 和 header action 行为。
```
...
最新自动化结论：SRK header meta DOM parity 已纳入收口记录，完整 gate 已通过。
