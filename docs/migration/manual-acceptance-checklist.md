...
```text
2026-05-27 最新记录：App shell site-switch trigger `px-2` class parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现站点切换按钮保留 8px padding 但缺少旧版 `px-2` class token，GREEN 验证按钮 class list 同时包含 `app-site-switch` 和 `px-2`，且左右 padding 仍为 `8px`。
```
