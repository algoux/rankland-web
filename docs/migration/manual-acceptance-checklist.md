...
```text
2026-05-28 最新记录：Collection collapse button wrapper DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration` 与 `git diff --check`。RED 复现 `[data-id="collection-collapse-button"]` 直接作为 `[data-id="collection-nav"]` 首个子节点，parent 为 `DIV.srk-collection-nav` 且带 nav style，而不是旧 React plain `DIV` 包裹；GREEN 验证 nav 首个直接子节点为无 class/inline style 的 plain `DIV`，collapse button 是该 wrapper 的直接子节点，并且 collection desktop/mobile bounds 仍通过。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client route，`test:migration` 通过 build、38 个 unit 文件 / 158 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、61 passed / 1 skipped full-chain Playwright tests。
```
...
最新自动化结论：Collection collapse button wrapper DOM parity 已纳入收口记录；focused RED/GREEN 已通过，完整 gate 已通过，`git diff --check` 已通过。
