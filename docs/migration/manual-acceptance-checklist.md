...
```text
2026-05-27 最新记录：SRK ref-link item span DOM parity 已通过 focused RED/GREEN、完整 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 full-chain Playwright tests）和 `git diff --check`；focused RED 复现可见 SRK header reference links 缺少旧 React item-level `SPAN` wrappers（收到 `[]`），GREEN 验证 `Official Site`、`, Mirror`、`, Statements` 均恢复为独立 `span` 项，同时保留外层 wrapper `span`、文本、颜色、间距和 dropdown 行为。
```
