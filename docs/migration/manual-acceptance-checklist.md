...
```text
2026-05-28 最新记录：user modal body product class parity 已通过 focused RED/GREEN、ranklist full-chain 9 tests、完整默认 `test:migration` 与 `git diff --check`。RED 复现迁移版 user modal body 仍暴露 Vue-only `rankland-user-modal-body` class；GREEN 验证 user modal body 恢复旧 React exact `user-modal` class list，且标题、无重复正文用户名、子节点 DOM/data-id hooks、照片宽度计算和正文文字颜色 selector 覆盖保持不变。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client route，`test:migration` 通过 build、36 个 unit 文件 / 154 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests。
```
