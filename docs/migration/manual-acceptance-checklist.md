...
```text
2026-05-27 最新记录：search state utility class parity 已通过 focused Search RED/GREEN、完整 Search full-chain、完整 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、56 full-chain Playwright tests）和 `git diff --check`。
```
