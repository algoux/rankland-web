...
```text
2026-05-28 最新记录：SRK controls wrapper product class parity 已通过 focused RED/GREEN、Ranklist/Live full-chain 20 tests、完整默认 `test:migration` 与 `git diff --check`。RED 复现迁移版 controls root 仍暴露 Vue-only `rankland-ranklist-controls` class；GREEN 验证 `[data-id="rankland-ranklist-controls"]` 恢复旧 React exact `mt-3 mx-4 flex justify-between items-center` class list，且 flex display、center alignment、space-between、12px top margin、16px horizontal margins 和 normal row/column gaps 保持不变。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，生成 6 个 client routes，build、36 个 unit files / 154 个 unit tests、1 个 SSR smoke test、1 个浅层 Playwright test、60 passed / 1 skipped full-chain Playwright tests 均通过。
```
