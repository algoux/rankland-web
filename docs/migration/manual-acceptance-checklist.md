...
```text
2026-05-27 最新记录：SRK footer external link rel omission parity 已通过 focused RED/GREEN、完整 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 full-chain Playwright tests）和 `git diff --check`；focused RED 复现 footer GitHub/Standard Ranklist/榜单合集外链仍渲染 Vue-added `rel="noreferrer"` 而旧 React 缺省 `rel`，GREEN 验证三处 footer package/project 外链恢复旧版 `target="_blank"` / 无 `rel` DOM，同时保留链接文本、href、footer root/paragraph 类、段落间距、联系入口和备案行为。
```
