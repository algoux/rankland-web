...
```text
2026-05-27 最新记录：SRK checker error DOM parity 已通过 focused RED/GREEN、完整 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 full-chain Playwright tests）和 `git diff --check`；focused RED 复现结构非法 object JSON 仍显示 renderer-error Alert，GREEN 验证旧版 `div.ml-8 > h3 + pre` 校验错误 DOM、32px 左边距、非空 `pre`，并确认该路径不显示 renderer-error Alert。
```
