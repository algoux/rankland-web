...
```text
2026-05-27 最新记录：Search section content DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、59 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 Vue-only `/search` 内容外层，title 带 `.search-section-title`、list 带 `.search-list`、empty state 带 `.search-empty-state`；GREEN 验证搜索页 result/recent title 恢复旧 `div.opacity-70`，list 恢复旧 `div.mt-2 > .ant-list`，empty state 恢复旧 `div.mt-2`，且无这些 Vue-only 类名。
```
