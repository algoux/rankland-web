...
```text
2026-05-27 最新记录：SRK check-error wrapper class parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 152 unit tests、1 SSR smoke test、1 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 Vue-only checker-error 外层类名 `rankland-ranklist-check-error ml-8`，旧 React 仅渲染 `class="ml-8"`；GREEN 验证 `/playground` 结构非法 object JSON 的 SRK checker-error 外层精确为 `ml-8`，仍保留旧版 `h3` 和非空 `pre`，不渲染 render-error Alert，也不请求 mock 后端。
```
...
最新自动化结论：SRK check-error wrapper class parity 已纳入收口记录，完整 gate 已通过。
