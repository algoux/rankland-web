...
```text
2026-05-28 最新记录：user modal marker label product class parity 已通过 focused RED/GREEN、ranklist full-chain 9 tests、完整默认 `test:migration` 与 `git diff --check`。RED 复现迁移版 marker label 仍暴露 Vue-only `rankland-user-modal-marker` class；GREEN 验证 marker label 恢复旧 React modal class + SRK preset class exact list `user-modal-info-marker srk-preset-marker-yellow`，且 marker 文本、preset 样式、inline-block、padding、border、radius、font-size 和 spacing 保持不变；完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client routes，`test:migration` 通过 build、36 个 unit files / 154 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests。
```
