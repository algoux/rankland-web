...
```text
2026-05-27 最新记录：App shell legacy layout/logo class parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现根布局缺少旧版 `layout`、header 内层缺少 `flex justify-between`、computed `justify-content` 为 `normal` 且 logo link 缺少内层 `.logo` DOM，GREEN 验证根布局 `layout`、header `flex justify-between` / `space-between`、64px `.logo` box、40px logo image 和既有 shell/site-switch/nav 行为。
```
