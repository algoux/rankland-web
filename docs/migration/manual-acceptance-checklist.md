...
- 最高优先级后续 slice：route-level product polish, if a new concrete visual difference is found.
...
```text
2026-05-28 最新记录：Collection menu open keys interaction parity coverage 已通过 focused collection full-chain 验证与完整默认 `test:migration`。新增覆盖确认 `/collection/official?rankId=test-key` 会自动展开 selected-path `dir-icpc` 子菜单，用户点击该子菜单标题可手动收起，selected leaf 隐藏，URL 仍保持 `/collection/official?rankId=test-key`。该 focused assertion 首次运行即通过，说明当前产品行为已匹配旧 React 交互，本切片不需要生产代码改动。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client route，`test:migration` 通过 build、38 个 unit 文件 / 158 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、61 passed / 1 skipped full-chain Playwright tests，`git diff --check` 通过。
```
...
- `[x]` `SITE_ALIAS=cnn` / `RANKLAND_SITE_ALIAS=cnn` 的全球站点分支不渲染中国站点专属 inline `word-break`
...
- `[x]` App shell 头部主导航保留旧版中间 `div` 包裹层，且该包裹层计算为 `flex: 1 1 0%`、`min-width: 0px`
- `[x]` App shell 头部右侧站点切换保留旧版 `RightMenu` plain `DIV` 包裹层，站点切换按钮不直接作为 `.app-header-inner` 子节点
...
- `[x]` raw app document head 保留旧版 `lang="zh-Hans"`、viewport、SEO/OpenGraph/Twitter meta、Dark Reader lock 和 fallback title
- `[x]` raw app document 保留旧版 body inline 防闪烁样式、Dark Reader 清理脚本，并在 app CSS 加载后恢复 body opacity 为 1
- `[x]` 长页面 BackTop 行为可接受
- `[x]` 系统主题同步行为可接受
...
- `[x]` SRK 合法 `result: null` 空状态单元格保留旧 React problem alias/title/index fallback 文本，不渲染 status-block class
...
- `[x]` SRK header 隐藏相关链接触发器不渲染 Vue-only `rankland-ranklist-ref-link-extra-action` 产品 class，仍保留 Ant Design runtime `ant-dropdown-trigger`
...
- `[x]` Ranklist NotFound、通用错误和 loading 状态外壳均保留旧版 plain `DIV.mt-16.text-center` DOM，不渲染 Vue-only `ranklist-state` class，loading 使用外层 `DIV` 包含 Ant Design Spin
- `[x]` 筛选和进度控制可用
...
- `[x]` SRK controls 根节点不添加 Vue-only gap，普通 Ranklist 无 extra-action slot 时仍保留旧版空 plain `DIV`，Live extra-action 外层保留旧版 plain `div` block chrome
...
- `[x]` 用户弹窗 wrap class 精确为旧版 `srk-modal-wrap`，不渲染 Vue-only `rankland-user-modal`
- `[x]` 用户弹窗内容根节点 class 精确为旧版 `user-modal`，不渲染 Vue-only `rankland-user-modal-body`
...
- `[x]` 用户弹窗团队成员分隔符 class 精确为旧版 `user-modal-info-team-members-slash`，不渲染 Vue-only `rankland-user-modal-team-separator`，并保留 raw ` / ` 文本、0.5 opacity 和 80% 字号
- `[x]` 用户弹窗团队成员保留旧版每个成员一个外层 `span` 的 item-level DOM 结构
- `[x]` 用户弹窗标记行 class 精确为旧版 `user-modal-info-markers mt-2`，不渲染 Vue-only `rankland-user-modal-markers`，并保留 marker label 文本、样式和 8px 顶部间距
- `[x]` 用户弹窗标记标签 class 精确为旧版 modal class + SRK preset class `user-modal-info-marker srk-preset-marker-yellow`，不渲染 Vue-only `rankland-user-modal-marker`，并保留 marker 文本、preset 样式和 label 间距
...
- `[x]` 用户弹窗奖区标签 class 精确为旧版 `user-modal-segment-label bg-segment-*`，不渲染 Vue-only `rankland-user-modal-segment-label`，并保留标签文本、背景色和 4px padding/圆角/白色文本样式
- `[x]` 用户弹窗标语节点 class 精确为旧版 `slogan mt-4 mb-2`，不渲染 Vue-only `rankland-user-modal-slogan`，并继续展示 `SLOGAN` 伪标签、旧版字体样式和 16px/8px 间距
- `[x]` 用户弹窗照片外壳保留旧版 `mt-4` 类和 16px 顶部间距，照片仍保持 full-width
- `[x]` 用户弹窗照片和标语共享旧版 `div.mt-4` 外壳结构
- `[x]` 用户弹窗无照片/无标语用户仍保留旧版空 `mt-4` 照片/标语外壳
- `[x]` rank-time 面板 class 精确为旧版 `mt-4`，不渲染 Vue-only `rankland-rank-time-panel`，rank-time curve 外层和 G2 容器不渲染 Vue-only `rankland-rank-time-*` 产品 class，curve 外层不渲染 Vue-only `data-chart-status` 生命周期属性，不展示额外单位、摘要或过题事件 chip，G2 曲线可接受
- `[x]` 导出 SRK 可用
- `[x]` 导出 Gym Ghost 可用
- `[x]` 导出 VJudge replay 可用
- `[x]` 导出 XLSX 可用
- `[x]` 分享链接正确，复制本页链接时保留旧版 focus-only query 过滤，并像旧 React `useCurrentUrl` 一样不复制 URL fragment
- `[x]` iframe 嵌入代码正确
...
- `[x]` SRK footer 根节点保留旧版 `DIV.text-center.mt-8` DOM 和段落 `mb-0` / `mt-1 mb-0` 工具类
...
- `[x]` 合集 NotFound、通用错误、loading、选中榜单错误和选中榜单 loading 状态外壳均保留旧版 plain `DIV.pt-16.text-center` DOM，不渲染 Vue-only `collection-state` class，loading 使用外层 `DIV` 包含 Ant Design Spin
- `[x]` 合集树或菜单可用
- `[x]` 分类或分组展示可接受
...
- `[x]` 合集 loaded shell 根容器不添加 Vue-only `position` / `min-height` chrome，computed `position` 为 `static`、`min-height` 为 `0px`
- `[x]` 合集导航 wrapper 保留旧版 `DIV.srk-collection-nav`，不渲染 Vue-only `aside` 或 `collection-nav` 产品 class
- `[x]` 合集菜单榜单叶子节点保留旧版 `Link` anchor DOM：`collection-menu-item-*` 为 `A`，带 collection `rankId` href，无 Vue-only `role` / `aria-current`，目录 label 仍为 `SPAN`
- `[x]` 合集菜单 selected-path 子菜单保留旧版可手动收起交互：`dir-icpc` 为当前 `rankId=test-key` 自动展开后，点击子菜单标题可收起、隐藏 selected leaf，且不改变当前 URL
- `[x]` 合集折叠按钮保留旧版 plain `DIV` 包裹层：`collection-nav` 首个直接子节点为无 class/inline style 的 `DIV`，`collection-collapse-button` 是该 wrapper 的直接子节点
...
```text
2026-05-28 追加复核：collection menu leaf anchor DOM 已补 RED/GREEN；RED 复现榜单叶子 label 仍为 `SPAN role="link"` 且 selected 时带 `aria-current="page"`、无 `href`，GREEN 验证旧 React `Link` anchor DOM 恢复，`href` 指向 `/collection/official?rankId=test-key`，无 Vue-only `role` / `aria-current`，目录 label 仍为 `SPAN`，mobile collapse 与 collection desktop/mobile bounds 保持通过。

2026-05-28 追加复核：collection menu open keys interaction parity 已补 full-chain 覆盖；focused assertion 直接通过，验证 selected-path `dir-icpc` 为 `rankId=test-key` 自动展开后可被用户手动收起，selected leaf 隐藏，URL 保持 `/collection/official?rankId=test-key`，因此无需生产代码改动。

2026-05-28 追加复核：collection collapse button wrapper DOM 已补 RED/GREEN；RED 复现 button 直接挂在 nav 下，parent 为 `DIV.srk-collection-nav` 且带 nav style，GREEN 验证旧版 plain `DIV` 包裹层恢复、无 class/inline style，collapse button 仍是该 wrapper 的直接子节点，collection desktop/mobile bounds 保持通过。

2026-05-28 追加复核：collection NotFound/error/loading 与 selected-ranklist error/loading 状态外壳旧版 plain `DIV.pt-16.text-center` DOM 已补 RED/GREEN；RED 复现 collection-level NotFound/error 仍为 `SECTION.collection-state.pt-16.text-center`，selected-ranklist error 仍额外包含 `collection-state`，loading 直接以 Ant Design Spin 作为状态根节点，GREEN 验证五个状态均恢复旧版 `DIV.pt-16.text-center`，不再渲染 Vue-only `collection-state` class，loading 外层 `DIV` 内含 Ant Design Spin，collection desktop/mobile bounds 保持通过。
...
2026-05-28 追加复核：collection loaded root chrome 合同已补 RED/GREEN；RED 复现迁移版 `[data-id="collection-content"]` 仍计算为 `position: relative` 并带 route-local root min-height，GREEN 验证旧版 loaded shell 根容器为 `position: static` / `min-height: 0px`，collection 全文件 11 tests 通过。

2026-05-28 追加复核：collection loaded Head full URL 合同已补 RED/GREEN；RED 复现迁移版 `og:url` / canonical 仍为 `/collection/official?rankId=test-key` 相对路径，GREEN 验证旧 React absolute URL 行为恢复为 `https://rl.algoux.org/collection/official?rankId=test-key`，并覆盖 loaded title 与 `og:title`。
```
...
- `[x]` Monaco `@change` 正常产品路径会在 250ms throttle 后同步草稿和预览解析状态，合法 SRK JSON 可像旧 React 一样实时刷新预览
- `[x]` 预览区可用
...
- `[x]` Playground Head 保持旧 React 无 canonical link 行为，只保留页面 title 和 `og:title`
...
- Monaco editor parity：`[x]` 接受当前编辑器 / `[ ]` 后续处理 / `[ ]` 阻塞（旧 minimap/default option chrome 和 live preview sync 已处理；旧 Monaco `0.34.0` 包版本不精确保留，synthetic Monaco editing 仍是测试 harness 限制）
- 更完整的 playground UX polish：`[x]` 接受当前实现 / `[ ]` 后续处理 / `[ ]` 阻塞

备注：

```text
2026-05-28 复核：`tests/e2e/full-chain/playground.spec.ts` 覆盖旧 React Head 无 canonical link 行为、旧 React route wrapper 普通 `DIV`、无 Vue-only `.playground-page` class、无 route-local `70vh` min-height、route root 在隐藏 hydration marker 后保留旧版无 class 客户端承载 `DIV`、该承载层直接包含核心容器 `DIV.srk-playground-container`、核心容器无 Vue-only `.playground-layout`、旧 React `.srk-playground-preview` 外壳类名、桌面 flex 布局、500px 编辑器、自适应预览区、Monaco minimap/default option chrome（`.minimap` 可见且宽度大于 0，Playground 仅保留旧版 `selectOnLineNumbers: true` option）、有效 SRK 预览外壳旧版 `div.mt-8.mb-8` DOM/class token 和 32px 间距、编辑器外壳无旧版不存在的额外边框/圆角、旧 React 不存在的 `Playground` 顶部标题栏和可见 `Preview` 按钮已移除、hydration marker 和 editor-ready marker 视觉隐藏、欢迎弹窗旧版右上角 `QuestionCircleOutlined` 文档提示语和图标、invalid JSON 标题旧版 `h3.mt-16.text-center` 类名、invalid JSON 快捷键 Tag 旧版 `mr-0` class token 和 0px 右/左间距、旧 React 预览区右上角 `div.absolute.right-4.top-4` 外壳及精确类名 `absolute right-4 top-4`、plain no-class `QuestionCircleOutlined` srk 文档锚点、入口位置、图标、地址、`target="_blank"` 和无 `rel` DOM/referrer 语义，以及结构非法 object JSON 的旧版 SRK checker-error `div.ml-8 > h3 + pre` DOM。

2026-05-28 追加复核：Playground live preview sync parity 已补 RED/GREEN；RED 复现迁移版缺少共享 preview-sync helper 且 Vue 页面未证明 Monaco live-change 会同步 `parseState`，GREEN 验证 `syncPlaygroundPreviewSource()` 从同一 source 同步 `draftSource` 和 valid/invalid `parseState`，并用 source guard 覆盖 Monaco `@change` throttled path、`Ctrl/Cmd + S` preview path 和 E2E hook 共用同一状态转换。
```
...
- `[x]` Live NotFound、通用错误和 loading 状态外壳均保留旧版 plain `DIV.mt-16.text-center` DOM，不渲染 Vue-only `live-state` class，loading 使用外层 `DIV` 包含 Ant Design Spin
- `[x]` 桌面端普通 live 布局可接受
- `[x]` 移动端普通 live 布局可接受
- `[x]` 桌面端实时布局可接受
- `[x]` 移动端实时布局可接受
...
| 演练场 Monaco/editor parity | `[x]` | `[ ]` | `[ ]` | Monaco minimap/default option chrome 和旧 React live valid-source preview sync 已按旧行为还原；Monaco 0.43 保留为已验证 Vue wrapper 依赖，旧 0.34 包版本不再精确保留 |
...
最新自动化结论：Collection menu open keys interaction parity coverage 已纳入收口记录；focused collection full-chain 已通过，完整 gate 已通过，`git diff --check` 已通过。

无当前已复现阻塞；Playground live preview sync parity、SRK null status cell fallback parity、User modal wrap class parity、SRK controls empty extra-action DOM parity、Collection Head full URL parity、Rank-time chart status attribute parity、Rank-time chart wrapper class parity、SRK share link hash omission parity、App document body bootstrap parity、App document head meta parity、Collection container chrome parity、SRK extra ref-link trigger product class parity、SRK header actions wrapper product class parity、SRK controls extra-action gap parity、SRK extra-action wrapper product class parity、App shell legacy layout/logo class parity、App shell site-switch trigger `px-2` class parity、App shell site-switch dropdown content class/style parity、Fallback 404 class token parity、SRK table wrapper class attribute parity、Playground preview wrapper DOM/class parity、Playground docs link anchor class parity、Playground docs link wrapper class parity、Playground docs link wrapper DOM parity、Playground docs link rel omission parity、Playground Monaco minimap/default option chrome parity、App shell site-switch rel omission parity、Beian link rel omission parity、Home external link rel omission parity、ContactUs trigger anchor DOM parity、SRK contributor link rel parity、SRK header ref-link rel parity、SRK header meta DOM parity、SRK header meta product class parity、SRK renderer top-level wrapper DOM parity、SRK render-error wrapper DOM/class parity、SRK banner image class/style parity、SRK remarks wrapper exact class parity、SRK contributor item span DOM parity、SRK ref-link item span DOM parity、Playground shortcut tag mr-0 class parity、Collection selected-ranklist pb-8 class parity、Collection wrapper DOM parity、Collection hidden-header title style parity、Route content utility class parity、Ranklist loaded wrapper DOM parity、Live route wrapper chrome parity、Live root wrapper DOM parity、Live content wrapper DOM parity、Live canonical link omission parity、Live realtime Toastify DOM parity、Live scroll-solution unknown result class parity、Live scroll-solution order parity、Live mobile scroll toggle DOM parity、SRK checker error DOM parity、SRK check-error wrapper class parity、user modal empty organization line parity、user modal body product class parity、user modal organization line product class parity、user modal team members product class parity、user modal team separator product class parity、user modal markers product class parity、user modal marker label product class parity、user modal rank-time panel product class parity、rank-time chart wrapper class parity、user modal segment line product class parity、user modal segment label class parity、user modal slogan product class parity、user modal unofficial line product class parity、user modal photo wrapper product class parity、SRK modal root class parity、SRK modal table-wrapper DOM parity、SRK table spacer DOM parity、SRK header action display parity、SRK header action gap parity、SRK header action attribute parity、SRK header action status removal parity、SRK extra ref-link spacing parity、SRK view-count utility-class parity、SRK progress wrapper utility-class parity、SRK progress wrapper product class parity、SRK controls wrapper product class parity、SRK footer wrapper product class parity、user modal empty photo wrapper parity、SRK asset image error parity、Contact QQ image class parity、SRK header utility class parity、SRK header action utility class parity、SRK header title typography parity、header text size parity、header view-count fallback parity、SRK controls utility class parity、SRK remarks wrapper utility class parity、SRK footer utility class parity、SSR hydration marker visual parity、search shell DOM parity、search heading DOM parity、search input DOM parity、search list item DOM parity、search result count DOM parity、search keyword whitespace parity、search state wrapper DOM parity、search section content DOM parity、search row content DOM parity、search error message DOM parity、search state utility class parity、search error DOM parity、search list utility class parity、Playground legacy shell class parity、Playground invalid prompt class parity、user modal root class parity、user modal organization line class parity、user modal team members class parity、user modal team separator raw text coverage、user modal team-member entry DOM parity、user modal markers class parity、user modal marker label class parity、user modal rank-time panel class parity、user modal unofficial line class parity、user modal segment line class parity、user modal segment label class parity、user modal slogan spacing class parity、user modal photo wrapper class parity、user modal photo/slogan shared wrapper DOM parity 和 user modal photo inline width parity 已补充，保留 route polish 作为后续 review-driven slice。
...
1. Route-level product polish, if route review finds a concrete remaining visual difference.
