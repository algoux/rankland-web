...
```text
2026-05-28 最新记录：Ranklist header action status removal parity 已通过 focused RED/GREEN、ranklist full-chain 9 tests、live full-chain 11 tests、完整默认 `test:migration`（build、35 unit files / 152 unit tests、1 SSR smoke test、1 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`。RED 复现 header 导出操作后仍渲染 Vue-only `[data-id="rankland-ranklist-action-status"]` 内联状态；GREEN 验证 SRK/Gym Ghost/VJudge Replay/Excel 导出继续保留文件名和内容覆盖，同时导出后不渲染 inline status node，分享成功仍使用 Ant Design Vue notification。
```
...
最新自动化结论：Ranklist header action status removal parity 已纳入收口记录，完整 gate 已通过。
