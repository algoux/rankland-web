...
```text
2026-05-27 最新记录：Search row content DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、59 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 Vue-only `/search` 行内容，title 带 `.search-row-title`、view-count 带 `.search-view-count`、created-at 带 `.search-created-at`；GREEN 验证搜索页 result/recent 行内容恢复旧 React `p.mb-0`、`span.ml-2.opacity-70`、`p.mb-0.opacity-50.text-sm` 精确类名，且无这些 Vue-only 行内容类。
```
