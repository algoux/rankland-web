...
```text
2026-05-28 最新记录：user modal segment label product class parity 已通过 focused RED/GREEN、ranklist full-chain 9 tests、完整默认 `test:migration` 与 `git diff --check`。RED 复现迁移版 segment label 仍暴露 Vue-only `rankland-user-modal-segment-label` class；GREEN 验证奖区标签恢复旧 React exact `user-modal-segment-label bg-segment-gold` class list，且文本、背景类、inline-block、4px padding、4px border-radius 和白色文本保持不变；完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client routes，`test:migration` 通过 build、36 个 unit files / 154 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests。
```
