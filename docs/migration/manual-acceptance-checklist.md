...
```text
2026-05-28 最新记录：ranklist extra-action wrapper product class parity 已通过 focused RED/GREEN、live full-chain 11 tests、完整默认 `test:migration` 与 `git diff --check`。RED 复现迁移版 extra-action wrapper 仍暴露 Vue-only `rankland-ranklist-extra-action` class；GREEN 验证 extra-action wrapper 恢复旧 React plain `div`，class list 为空，且稳定 `data-id`、block display、normal gaps、Live scroll-solution toggle 和 controls root chrome 保持不变。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client route，`test:migration` 通过 build、36 个 unit 文件 / 154 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests。
```
