...
```text
2026-05-28 最新记录：SRK render-error wrapper DOM parity 已通过 focused RED/GREEN、Playground full-chain 7 tests、完整默认 `test:migration`（生成 6 条 client route、build、36 unit files / 154 unit tests、1 SSR smoke test、1 shallow Playwright test、60 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`。RED 复现迁移版 SRK render-error wrapper 缺少旧版 inline `max-width: 400px; margin: 100px auto` 样式且仍带 Vue-only `rankland-ranklist-error` 产品 class；GREEN 验证 wrapper 恢复为旧版 plain `div role="alert"`、inline style、无 Vue-only class、无 scoped `.rankland-ranklist-error` CSS，同时保留既有 Ant Design Alert 覆盖。
```
