...
```text
2026-05-27 最新记录：App shell site-switch dropdown content class/style parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、58 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现站点切换中国站点下拉缺少旧版 inline `word-break: keep-all`、`mb-0`、`opacity-60`、`text-xs`，GREEN 验证 inline/computed `word-break: keep-all`、段落 class token/0px bottom margin、副标题 span `opacity-60 text-xs`、12px 字号、0.6 opacity、旧版文案，以及既有 href/target/rel/icon 外壳行为。
```
