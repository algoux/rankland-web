...
```text
2026-05-28 最新记录：Collection menu leaf anchor DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration` 与 `git diff --check`。RED 复现 `[data-id="collection-menu-item-test-key"]` 渲染为 `SPAN`，带 Vue-only `role="link"` / `aria-current="page"` 且无 `href`，而不是旧 React `Link` anchor；GREEN 验证 ranklist leaf menu label 渲染为 `A`，`href` 为 `/collection/official?rankId=test-key`，无 `role` / `aria-current`，directory label 仍为 `SPAN`，mobile collapse 行为与 collection desktop/mobile bounds 仍通过。完整 gate 使用 Node `v24.11.1`、pnpm `8.15.9`，`gen:client-router` 生成 6 条 client route，`test:migration` 通过 build、38 个 unit 文件 / 158 个 unit tests、1 个 SSR smoke test、1 个 shallow Playwright test、61 passed / 1 skipped full-chain Playwright tests。
```
...
最新自动化结论：Collection menu leaf anchor DOM parity 已纳入收口记录；focused RED/GREEN 已通过，完整 gate 已通过，`git diff --check` 已通过。
