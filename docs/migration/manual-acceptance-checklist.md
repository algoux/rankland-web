...
```text
2026-05-27 最新记录：Search shell DOM parity 已通过 focused RED/GREEN、完整默认 `test:migration`（build、35 unit files / 151 unit tests、1 SSR smoke test、1 shallow Playwright test、59 passed / 1 skipped full-chain Playwright tests）和 `git diff --check`；RED 复现 Vue-only `/search` 外壳，`[data-id="search-page"]` 为 `MAIN.search-page`、内容 wrapper 为 `SECTION.search-panel`，且 root 带 `min-height: 70vh`；GREEN 验证搜索页已恢复旧 React `div.normal-content > div` 外壳 DOM，同时保留 CSR listAll、隐藏 hydration marker、Ant Design 搜索框和最近榜单行为。
```
