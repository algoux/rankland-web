...
- Current slice: SRK table wrapper offset parity
- Latest slice commit: `feat: 收口榜单表格缩进一致性` (this commit)
- Last recorded full gate: `corepack pnpm test:migration` passed build, 28 unit files / 138 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 41 full-chain Playwright tests across all migrated public routes during 2026-05-26 SRK table wrapper offset parity pre-commit verification
...
| `/ranklist/:id` | SSR | Foundation + shared wrapper header metadata/hover dropdown parity + Ant Design Vue filter controls + table-wrapper offset parity + G2 rank-time tooltip/animation parity + visual review + completion audit verified | `migration/live-page-foundation` | Unit, route, full-chain E2E including view count, contributors, reference links, hover export/share dropdown open-close, Ant Design Vue Select/Switch/Radio filter controls and official-only semantics, standalone table-wrapper `ml-4` offset coverage, G2 rank-time chart metadata/canvas coverage, footer contact modal, ranklist desktop/mobile screenshot review, `test:migration` | Product polish after route parity review |
...
| `/live/:id` | CSR | Foundation + parity follow-ups + reconnect/backoff + Toastify animation/pixel parity + shared Ant Design Vue filter controls + table-wrapper offset parity + G2 rank-time tooltip/animation parity + visual review + completion audit verified | `migration/live-page-foundation` | Unit, route, full-chain E2E including NotFound, WebSocket error reconnect, unexpected WebSocket close reconnect, scroll-solution toggle close, mobile toggle visibility, hidden internal status marker, desktop/mobile realtime layout bounds, legacy Toastify container/Zoom presentation, shared Ant Design Vue filter interaction, standalone table-wrapper `ml-4` offset coverage, user modal G2 rank-time chart metadata/canvas coverage, normal live desktop/mobile screenshot review, mobile progress label bounds, and `test:migration` | Product polish after route parity review |
