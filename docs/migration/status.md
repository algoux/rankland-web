...
- Current slice: live scroll-solution toggle close full-chain coverage
- Latest slice commit: `test: 补齐实时榜单滚动提交关闭覆盖` (this commit)
- Last recorded full gate: `corepack pnpm test:migration` passed for live scroll-solution toggle close coverage
...
| `/live/:id` | CSR | Foundation + parity follow-ups verified | `migration/live-page-foundation` | Unit, route, full-chain E2E including NotFound, WebSocket error, and scroll-solution close | Product review, realtime reconnect edge cases |
...
- Live realtime behavior has deterministic success, NotFound, WebSocket error, and scroll-solution close coverage, but production reconnect edge cases still need review.
