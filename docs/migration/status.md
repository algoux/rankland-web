...
- Current slice: Rank-time G2 tooltip/animation parity
- Latest slice commit: `feat: 收口排名时间图表一致性` (this commit)
- Last recorded full gate: `corepack pnpm test:migration` passed build, 27 unit files / 132 unit tests, 1 SSR smoke test, 1 shallow Playwright test, and 39 full-chain Playwright tests across all migrated public routes during 2026-05-26 Rank-time G2 parity pre-commit verification
- Next recommended focus: collection remaining-height/pixel polish
...
| `/ranklist/:id` | SSR | Foundation + shared wrapper header metadata/hover dropdown parity + G2 rank-time tooltip/animation parity + visual review + completion audit verified | `migration/live-page-foundation` | Unit, route, full-chain E2E including view count, contributors, reference links, hover export/share dropdown open-close, G2 rank-time chart metadata/canvas coverage, footer contact modal, ranklist desktop/mobile screenshot review, `test:migration` | Product polish after route parity review |
...
| `/live/:id` | CSR | Foundation + parity follow-ups + reconnect/backoff + Toastify animation/pixel parity + G2 rank-time tooltip/animation parity + visual review + completion audit verified | `migration/live-page-foundation` | Unit, route, full-chain E2E including NotFound, WebSocket error reconnect, unexpected WebSocket close reconnect, scroll-solution toggle close, mobile toggle visibility, hidden internal status marker, desktop/mobile realtime layout bounds, legacy Toastify container/Zoom presentation, user modal G2 rank-time chart metadata/canvas coverage, normal live desktop/mobile screenshot review, mobile progress label bounds, and `test:migration` | Product polish after route parity review |
...
| SRK Vue wrapper | Foundation + live controls + modal clicks + G2 rank-time user modal + asset URL rewriting + basic export/share + converter-backed exports + footer contact prompt + header metadata + Ant Design Vue hover dropdowns + collection selected-wrapper controls done | Remaining exact table pixel parity is product/visual review driven |
...
1. Collection product polish: exact remaining-height calculation and pixel animation parity, if product review requires it.
