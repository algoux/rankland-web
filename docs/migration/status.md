...
- Current slice: contact us modal parity
- Latest slice commit: `feat: 迁移联系弹窗入口` (this commit)
- Last recorded full gate: `FULL_CHAIN_APP_PORT=3210 FULL_CHAIN_MOCK_PORT=3211 corepack pnpm test:e2e:full-chain -- tests/e2e/full-chain/home.spec.ts tests/e2e/full-chain/ranklist.spec.ts` passed 3 full-chain tests
- Next recommended focus: continue app shell styling/theme polish or move to page-specific product review

## Route Progress

| Route | Render | Status | Branch / Slice | Coverage | Remaining parity / risk |
| --- | --- | --- | --- | --- | --- |
| `/` | SSR | Foundation + app shell + contact modal verified | `migration/live-page-foundation` | SSR smoke, shallow E2E, full-chain E2E, app shell full-chain E2E, contact modal full-chain E2E | Exact Ant Design shell styling, broader SEO consistency |
...
| Vue app shell | Foundation + contact modal verified | Logo, primary navigation, site switch, BackTop, legacy focus-mode shell bypass, and shared contact modal are covered by full-chain E2E. Exact Ant Design/GA/dark-theme parity remains product polish. |
...
- Whether app shell polish should prioritize exact Ant Design styling, analytics/theme parity, or page-level visual review.

## Known Risks

- Route foundations preserve core behavior but do not yet guarantee exact visual parity with old React/Ant Design pages.
- Vue app shell now preserves the basic legacy chrome, focus-mode bypass, and contact modal, but exact Ant Design menu/dropdown styling, GA pageview dispatch, Bowser-specific optimization, and dark-theme parity are still deferred.
...
1. App shell polish: exact styling, analytics/theme behavior, mobile visual review.
