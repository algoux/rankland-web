...
- Current slice: live WebSocket close parity full-chain coverage
- Latest slice commit: `test: 补齐实时榜单 WebSocket 关闭覆盖` (this commit)
- Last recorded full gate: `corepack pnpm test:migration` passed for live WebSocket close parity coverage
...
| `/live/:id` | CSR | Foundation + parity follow-ups verified | `migration/live-page-foundation` | Unit, route, full-chain E2E including NotFound, WebSocket error, unexpected WebSocket close, and scroll-solution toggle close | Product review, realtime reconnect policy |
...
- Live realtime behavior has deterministic success, NotFound, WebSocket error, unexpected WebSocket close, and scroll-solution toggle close coverage, but production reconnect policy still needs review.
