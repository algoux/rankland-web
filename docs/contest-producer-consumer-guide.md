...
业务 SSE 帧只包含 `event: events-available` 和 `data`，不发送 SSE `id:` 字段，也不携带事件
payload。服务端约每 15 秒额外发送一次 `: heartbeat\n\n` comment；`EventSource` 会忽略 comment，
它不表示有新事件。通知允许丢失、重复和乱序，消费者不依赖 `Last-Event-ID`，最终事件恢复依赖
HTTP catch-up。

Redis 正常且连接所在实例已经确认 `SUBSCRIBE` ACK 时，跨实例通知目标为 MySQL commit 后 1 秒内到达。
Redis 断线、丢消息或写实例在 commit 后、publish 前退出时，服务端每 5 秒从 MySQL 校准本机活跃
比赛；只有在 MySQL 健康且权威查询在 1 秒内完成时，故障路径才以 6 秒为验收上限。消费者算法
仍必须按“提示可能丢失、重复、乱序”设计，不能把该时延目标当成事件交付协议。
...
说明本地事件流版本或 cursor 已经失效。消费者应切换到一个新的本地 generation，清空事件缓存，
保存响应中的 `streamRevision` 和 `checkpointEventId=0`，然后从 `afterEventId=0` 重新 catch-up。

### SSE 重连和并发通知

SSE 初始通知可能已经领先于本地 cursor；catch-up 过程中也可能继续收到新的 SSE 通知。消费者不需要拼接 SSE 事件队列，因为 SSE 没有事件 payload，只需要维护一个目标高水位。

推荐逻辑如下。`generation` 是消费者本地的异步请求栅栏，不是服务端字段：
...
let streamRevision: number | undefined = loadStreamRevision();
let targetLatestEventId = localEventId;
let catchingUp = false;

function replaceGeneration(next: Watermark) {
  generation += 1;
  clearLocalEvents();
  streamRevision = next.streamRevision;
  localEventId = 0;
  // 新 revision 的 event id 从 0 重新开始，不能与旧 revision 的 target 做 Math.max。
  targetLatestEventId = next.latestEventId;
  saveCursor(localEventId, streamRevision);
}

function acceptWatermark(next: Watermark): boolean {
  if (streamRevision !== undefined && next.streamRevision < streamRevision) {
    return false;
  }
  ...
  if (streamRevision === undefined || next.streamRevision > streamRevision) {
    ...
    // 只有同 revision 的 event id 才可直接比较。
    targetLatestEventId = Math.max(targetLatestEventId, next.latestEventId);
  }

  return true;
}

function onEventsAvailable(payload: Watermark) {
  if (acceptWatermark(payload)) void ensureCatchUp();
}
...
async function ensureCatchUp() {
  ...
  try {
    while (streamRevision !== undefined && localEventId < targetLatestEventId) {
      const requestGeneration = generation;
      const requestRevision = streamRevision;
      const page = await fetchContestEvents({
        afterEventId: localEventId,
        streamRevision: requestRevision,
        compactProgress: false,
      });

      // reset/SSE 可能在请求期间切换 generation。旧响应的 events、cursor 和 latest id 必须整页丢弃。
      if (generation !== requestGeneration || streamRevision !== requestRevision) {
        continue;
      }

      if (page.resetRequired) {
        // afterEventId 越界时同 revision 也可能 resetRequired，因此这里必须显式新建 generation。
        if (page.streamRevision >= requestRevision) {
          replaceGeneration({
            streamRevision: page.streamRevision,
            // 保留请求期间收到的同 revision 新高水位；跨 revision 时不能沿用旧 target。
            latestEventId:
              page.streamRevision === requestRevision
                ? Math.max(targetLatestEventId, page.latestEventId)
                : page.latestEventId,
          });
        }
        continue;
      ...
      // 非 reset 响应也不允许把另一个 revision 的内容应用到当前状态。
      if (page.streamRevision !== requestRevision) continue;
    ...
    catchingUp = false;
    if (streamRevision !== undefined && localEventId < targetLatestEventId) {
      void ensureCatchUp();
    }
  }
}
...
- 发起 catch-up 时捕获本地 `{generation, streamRevision}`；返回时任一值变化就丢弃整个响应。
- 事件应用应按 eventId 升序处理。
- 对重复事件要幂等。
- 低 `streamRevision` 通知直接忽略；高 revision 清空旧状态并把 target 直接设为新 revision 的
  `latestEventId`；只有同 revision 才对 target 做 `Math.max`。
- `resetRequired` 使用同一 generation 切换逻辑，即使响应 revision 与请求相同也要清空失效 cursor。
...
- SSE 断线后由 `EventSource` 按服务端 `retry: 1000` 自动重连；重连后初始通知会再次给出当前高水位。
- heartbeat 是 comment，只维持连接，不应触发 catch-up 或修改 target。
...
事件中的 solution result / previousResult 应尽量使用原始评测结果，不等同于 SRK `SolutionResultLite` / `SolutionResultFull` 的完整集合。`FZ`（SRK `?`，封榜/未知展示态）是 deprecated 的展示兜底值；当数据源无法获取封榜期间的原始结果时仍可使用，但能拿到原始结果时不要主动写入 `FZ`。`FB` 是根据原始 AC 和榜单状态计算出的 first blood 属性，不允许出现在 append 或 get events 的 result 字段中。
...
14. 事务提交后先通知写实例的本机 SSE Hub，再 best-effort 发布 Redis 供其他实例扇出；通知平面
    失败不改变已经提交的 append 响应。
...
- eventId 已存在但 payload hash 不同：返回 `ErrCode.ContestEventIdConflict`（`100006`），事务回滚。
- eventId 不接续：返回 `ErrCode.ContestEventIdGap`（`100005`），事务回滚。
...
不要在不确定提交结果时重新生成同 eventId 的不同 payload，否则会触发 `ErrCode.ContestEventIdConflict`（`100006`）。
...
| `ErrCode.ContestEventInvalidBatch` (`100003`) | 停止脚本，修生产逻辑 |
| `ErrCode.ContestEventProducerLocked` (`100004`) | 停止脚本，确认是否已有 producer；必要时管理员 release |
| `ErrCode.ContestEventStreamRevisionMismatch` (`100007`) | 停止脚本，丢弃本地旧 cursor，重新获取 stream state 后从 eventId 1 生产新 revision |
| `ErrCode.ContestEventIdGap` (`100005`) | 停止脚本，重新对齐服务端 `lastEventId` |
| `ErrCode.ContestEventIdConflict` (`100006`) | 停止脚本，说明同 eventId 的 payload 不一致 |
| `ErrCode.ContestNotFound` (`100001`) | 停止脚本，确认 `uk` 是否正确 |
...
- producer lock 被释放。
- 服务端宣布新的 `streamRevision` 和 `latestEventId=0` 高水位。

消费者只在看到更高 revision 或权威 catch-up 返回 `resetRequired` 时切换 generation、清本地缓存，
再从 `afterEventId=0` 拉新流；晚到的低 revision 通知直接忽略。
