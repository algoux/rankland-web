...
```text
GET /api/v2/contests/:uk/events?afterEventId=<local>&streamRevision=<revision>
```
...
1. 从本地缓存读取 `lastEventId`、`streamRevision` 和已缓存事件；没有缓存则使用 `lastEventId=0`。
2. 建立 SSE 连接。
3. 立即执行一次 HTTP catch-up。
4. 如果返回 `hasMore=true`，继续分页拉取。
5. 处理完响应后，把本地 cursor 更新为 `checkpointEventId`。
...
```text
GET /api/v2/contests/contest-a/events?afterEventId=0
```
...
```text
GET /api/v2/contests/contest-a/events?afterEventId=0&compactProgress=false
```
...
```text
GET /api/v2/contests/:uk/events?afterEventId=<lastEventId>&streamRevision=<streamRevision>
```
...
{ "events": [/* BatchProducerEvent.events */] }
```

两种方式都提交 `BatchProducerEvent`。
...
7. 校验/设置 producer lock。
8. 对本批所有 eventId 做一次批量查询。
9. 在内存中检查 duplicate/gap/conflict。
10. 收集真正新增的事件。
11. 对新增事件做一次 bulk insert。
12. 更新 `contest_event_stream.last_event_id`。
13. 事务提交后广播 SSE 高水位通知。
...
数据库唯一约束是 `(contest_id, event_id)`。reset/dropEvents 会先删除当前 contest 的旧 `contest_event` 行，再递增 `streamRevision`，因此新事件流可以重新从 eventId 1 开始。
...
while (running) {
  ...
  const batchBytes = encodeBatchProducerEvent(events);
